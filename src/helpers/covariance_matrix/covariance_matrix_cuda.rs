...
pub fn calculate_covariance_matrix_cuda(
    ...
    let cov_ptx = compile_ptx(COVARIANCE_KERNEL).context(
        "Could not compile the CUDA Kernel \
    source code to PTX",
    )?;
    ...
    let kernel = dev
        .get_func("covariance_kernel", "covariance_kernel")
        .context("Failed to get CUDA kernel function")?;

    // Load data into the kernel
    let d_returns = dev.htod_copy(data.to_vec()).context(
        "Failed to copy returns data to the \
    device",
    )?;
    let d_means = dev
        .htod_copy(means)
        .context("Failed to copy means data to device")?;

    // Load the covariance matrix (size: n x n)
    let d_cov_matrix = dev
        .alloc_zeros::<f32>(n_assets * n_assets)
    ...
    let grid_dim = (
        (n_assets as u32).div_ceil(block_dim.0),
        (n_assets as u32).div_ceil(block_dim.1),
        1,
    );
